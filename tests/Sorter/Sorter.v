...
fn main() {	
	...
	max_iters := fp.int('max-iters', `i`, 100, 'How deep should we crawl?')
	crawl_out_of_scope := fp.bool('crawl-out-of-scope', `z`, true, 'Should we crawl out of scope sites?')
	show_out_of_scope := fp.bool('show-out-of-scope', `s`, true, 'Should we show out of scope sites in the results?')
	...
	urls := ["https://vlang.io/","https://www.google.com"]
...
fn crawl(in_scope_urls []string, max_iters int, crawl_out_of_scope bool, show_out_of_scope bool, success_status_codes []int) ?int {
	mut urls := []string{}
